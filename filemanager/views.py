...
@login_required
def map_line_view(request, file_id=None):
    if file_id: 
        uploaded_file = UploadedFile.objects.get(id=file_id, user=request.user)
        path = uploaded_file.file.path
    else:
        path = os.path.join(settings.MEDIA_ROOT, 'default.geojson')
...
def dashboard(request):
    file_path = os.path.join(settings.MEDIA_ROOT, 'default.geojson')
    if os.path.exists(file_path):
        file_exists = True
        file_url = os.path.join(settings.MEDIA_URL, 'default.geojson')
    else:
        file_exists = False
        file_url = None
    context = {
